...
class SurfaceIPSatellite extends EventEmitter {
	...
	constructor(deviceInfo) {
		...
		this.#scaleX = deviceInfo.scaleX
		this.#scaleY = deviceInfo.scaleY
		...
		if (this.socket !== undefined) {
			...
			if (this.#streamBitmaps) {
				if (buffer === undefined || buffer.length != 15552) {
					...
					let image = imageRs.ImageTransformer.fromBuffer(buffer, 72, 72, imageRs.PixelFormat.Rgb)

					if (this.#scaleX > 0 && this.#scaleY > 0) {
						image = image.scale(this.#scaleX, this.#scaleY)
					}

					const rotation = translateRotation(this._config.rotation)
					if (rotation !== null) image = image.rotate(rotation)

					let newbuffer = Buffer.from(await image.toBuffer(imageRs.PixelFormat.Rgb))
					params += ` BITMAP=${newbuffer.toString('base64')}`
				}
			}
			if (this.#streamText) {
